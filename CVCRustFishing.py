...
def get_rod_health(hotbar_slot): # Get the health of the fishing rod
    ...
    # Find health
    for i in range(h):
        if(health_bar[i, 0] > 100 and health_bar[i, 0] < 130):
...
def get_hotbar(): # Find where fishing rod is positioned
    ...
    for c in range(columns):
        ...
        if("rod" in slot_value):
            ...
            # *** returning first rod currently ***
            return left_current, top_current, w, h, rod_health, has_bait
    return hotbar
...
def drag_and_drop(x1, y1, x2, y2, queue_m): # Move mouse to x1y1 and drag and drop at x2y2
    ...
    dx2, dy2 = x2 - x1 , y2 - y1
    # Random Move time in range
    total_time = randint(700,750)
    # Move mouse to item
    lerp(total_time, total_time/2, dx1, dy1, time.perf_counter(), queue_m)
    # Put left mouse press in queue
    queue_m.put([0,0,2])
    # Drag item to destination
    lerp(total_time, total_time/2,  dx2, dy2, time.perf_counter(), queue_m)
    # Wait then release
    sleep_time(200)
...
def restock_rod(inventory, rod_x, rod_y, queue_m): # Attempt to restock the fishing rod
...
def find_buttons():
    ...
    for i in range(len(gut_p)):
        ...
        # Check if gut equals "gut", and drop equals "drop"
...
def gut_fish(inventory, strict, queue_m): # Gut all the fish in inventory that aren't sharks, trout, or salmon
    # Fish to gut
    ...
    for r in range(rows):
        for c in range(columns):
            ...
            # If shark, salmon, ignore(unless strict on)
            ...
            # Get randomized time
            total_time = randint(450,550)
            # Move mouse to item
            lerp(total_time, total_time/2, dx, dy, time.perf_counter(), queue_m)
            # Click on item
            queue_m.put([0,0,1])
            # Sleep for 100ms
            sleep_time(200)
            ...
            while(gut_coordinates != -1 and drop_coordinates != -1):
                ...
                # Click "Gut" 5 times
                queue_m.put([0,0,1])
                # Sleep for 300ms sub 200ms cant reconize
                sleep_time(200)
                # Get next loop
                gut_coordinates, drop_coordinates = find_buttons()
...
def cast_fishingrod(queue_m, fish_images_path, fish_images): # Cast the fishing rod
    # Rod casting timing
    ...
    sleep_time(300)
    # Get hotbar, returns position of sihing rod, health, and if it has bait 
    left, top, w, h, health, has_bait = get_hotbar()
    ...
    if(not has_bait):
        ...
        # Attempt to restock fishingrod
        success = restock_rod(inventory, rod_center_x, rod_center_y, queue_m)
        # If it wasn't able to, lack of bait
        if not success:
            # Then gut fish
            gut_fish(inventory, True, queue_m)
            # Wait a second
            sleep_time(200)
            # Get inventory
            inventory = get_inventory(fish_images_path, fish_images)
            # Try and restock rod a second time
            success = restock_rod(inventory, rod_center_x, rod_center_y, queue_m)
        # If it still wasn't able to, lack of bait and no low tier fish to gut
        if not success:
            # Allow Salmon to be gutted
            gut_fish(inventory, False, queue_m)
            # Wait a second
            sleep_time(200)
            # Get inventory
            inventory = get_inventory(fish_images_path, fish_images)
            # Then try and restock rod a second time
            success = restock_rod(inventory, rod_center_x, rod_center_y, queue_m)
        # Close inventory
        send_keystroke(0x09)
        ...
        # Close inventory
        send_keystroke(0x09)
    print(left, top, w, h, health, has_bait)
    ...
    # Sleep for 7 to 8 seconds to wait for rod to reset
    sleep_time(randint(min_rod_reset, max_rod_reset))
...
def fish(queue_m, queue_f): # Fishing main loop
    ...
    min_press, max_press = 420, 450
    min_release, max_release = 390, 420
    # Auto fishing toggle
    fishing = False
    press = False
    release = False
    ...
    # Store total number of time per fish, to find average fish time
    sum_amount_time = 0
    ...
    while True:
        ...
        if(win32api.GetAsyncKeyState(0x26) < 0):
            while(win32api.GetAsyncKeyState(0x26) < 0):
                pass
            # Get hotbar(IE rods)
            left, top, w, h, health, has_bait = get_hotbar()    #left_current, top_current, w, h, rod_health, has_bait
            # Set rod center
            rod_center_x, rod_center_y = int(left + w/2), int(top+h/2)
            # If the current rod doesn't have bait
            if(not has_bait):
        ...
        try:
            ...
            print("Average catch time: ", sum_amount_time/fish_count)
            print("All fish: ", fish_caught)
        except:
            pass        # No new fish caught

        # If its been longer than 1 minute 30 without catching a fish, re-cast
        if((time.perf_counter() - fish_timer) > 90):
            # Set casting to true
            cast_rod = True
            fish_timer = time.perf_counter()
            print("Fish timer > 2 minutes - Recasting")

        # If rod needs to be cast
        if(cast_rod):
            # Sleep for delay
            sleep_time(1000)
            # Cast rod
            cast_fishingrod(queue_m, fish_images_path, fish_images)
            # Restart cast
            cast_rod = False
        ...
        if(((time.perf_counter() - start_timer) * 1000) > time_to_sleep):
            ...
            if(press):
                ...
                #print("Hold for:", time_to_sleep,"ms")
                continue
            ...
            if(release):
                ...
                #print("Release for:", time_to_sleep,"ms")
                continue
